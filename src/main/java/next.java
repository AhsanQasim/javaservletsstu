...
public class next extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
         ...
         String sql2;
         ResultSet rs;
         if(globalnext.globalnext<count)
         {
             sql2 = "SELECT * FROM address.address LIMIT "+globalnext.globalnext+",1";
             ...
             globalnext.globalnext=globalnext.globalnext+1;
         }else {
        	 String limit="No More Values";
        	 request.setAttribute("limiti", limit);
             request.getRequestDispatcher("/tryindex.jsp").forward(request, response);
             System.out.println("dispatched!");
         }
